...
class ClearDataTool(CommandTool):
    """
    button to clear any data from graph
    """
    def __init__(self, manager, toolbar_id=DefaultToolbarID):
        CommandTool.__init__(self, manager,title='Clear',tip="Clear All Plot Data",
                             icon = get_icon("delete.png"),
                             toolbar_id=toolbar_id)
    def activate_command(self, plot, checked):
        """Clear Data"""
        # print "Clear Data Activated"
...
class GuiQwtPlot( CurveWindow ):
    ...
    def __init__( self ):
        ...
        cleartool = self.add_tool(ClearDataTool)
        self.connect(cleartool, SIGNAL( 'clear' ), self.cleardata)
    ...
    def cleardata(self):
        ...
        """
        print('cleardata call')
        self.plot.del_items(self.plot.get_items(False, ICurveItemType))
        self.plot.replot()
